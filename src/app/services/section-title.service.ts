...
export class SectionTitleService {
  ...
  getSelectedCriteria(indicatorPrefix: string,
    criteria: CriteriaContract,
    isZoneRequired: boolean = true,
    isDistrictRequired: boolean = true,
  ...
  private getSelectedMunicipality(prefix: string, criteria: CriteriaContract): string {
    ...
    const lookupMap = this.lookupService[prefix + "MunicipalitiesMap" as keyof LookupService] as Record<number, Lookup>;
  ...
  private getSelectedPropertyType(prefix: string, criteria: CriteriaContract): string {
    ...
    return criteria.propertyTypeList &&
      criteria.propertyTypeList[0] !== -1
  ...
  private getSelectedPurpose(prefix: string, criteria: CriteriaContract): string {
    ...
    return criteria.purposeList &&
      criteria.purposeList[0] !== -1
      ? lookupMap[criteria.purposeList[0]].getNames()
      : '';
  }
